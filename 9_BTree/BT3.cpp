...
void BTree::split(node *x, int i){
    ...
    node *z = new node(y->leaf);                                                //z will be the right side
    z->n = t/2;
...
void BTree::merge(node* p, int posT){
    ...
    for(int i=position; i< p->n -1; i++){                                       //shift parent data left
        ...
        p->C[i+1] = p->C[i+2];                                                  //shift parent's child pointers
    }
    p->n --;
    for(int i = 0; i < right->n; i++){
        left->keys[left->n + 1 + i] = right->keys[i];                           //merging of child keys
        left->C[left->n + 1 + i] = right->C[i];                                 //merging of child pointers
        if(right->C[i] != NULL){
            right->C[i]->parent = left;                                         //resetting parents
        }
    }
    left->C[left->n + 1 + right->n] = right->C[right->n];                       //this case was left out in the loop
    if(right->C[right->n] != NULL){
        right->C[right->n]->parent = left;
    }
    left-> n = left->n + right->n + 1;                                          //new size of node

    delete right;
    if(p == root){                                                              //root must be reset
        if(p->n == 0){
            delete root;
            root = left;
            left->parent = NULL;
